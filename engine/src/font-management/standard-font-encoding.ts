...
import type { StandardFontMetadata, StandardPostscriptFontName } from './sentinel';

type ProxyBBoxTuple = readonly [number, number, number, number];
...
const toHexByte = (value: number): string => `00${Math.max(0, Math.min(255, value)).toString(16)}`.slice(-2);
...
export const encodeStandardFontText = (
    ...
    for (const character of text || '') {
        ...
        glyphs.push({
            ...
            bbox: supported ? metrics.bboxByCode[codePoint] : undefined,
            supported
        });
        positions.push({
...
export const encodeStandardFontTextToHex = (
    metadata: StandardFontMetadata | StandardPostscriptFontName,
    text: string
): string[] => encodeStandardFontText(metadata, text).glyphs.map((glyph) => toHexByte(glyph.encodedByte));
