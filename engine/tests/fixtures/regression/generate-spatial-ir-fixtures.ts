...
interface SpatialDocumentFixture {
...
function getPageGeometry(document: DocumentIR): SpatialDocumentFixture['pageGeometry'] {
...
function compilePageRegions(
    context: NormalizeContext,
    role: 'header' | 'footer',
    definition?: PageRegionDefinition
): CompiledPageRegionSet | undefined {
...
function compilePageOverrides(
    context: NormalizeContext,
    rawOverrides: unknown
): FlowBlock['pageOverrides'] | undefined {
    ...
    const compileOne = (role: 'header' | 'footer', region: unknown): CompiledPageRegion | null | undefined => {
        ...
        const pageRegion = region as PageRegionContent;
        const insetTop = role === 'header'
            ? LayoutUtils.validateUnit(context.document.layout.headerInsetTop ?? 0)
            : LayoutUtils.validateUnit(context.document.layout.footerInsetTop ?? 0);
        const insetBottom = role === 'header'
...
function normalizeElement(
    ...
    context: NormalizeContext,
    scope: ContentScope
): ZoneContentItem | ZoneContentItem[] {
...
function normalizeZoneMap(
    ...
    const trackDefinitions = Array.isArray(zoneOptions.columns) && zoneOptions.columns.length > 0
        ? zoneOptions.columns
        : Array.from({ length: declaredZones.length }, () => ({ mode: 'flex', fr: 1 } as TableColumnSizing));
    ...
    const zones = declaredZones.map((zone, index) => {
...
function normalizeTable(
    ...
    const trackDefinitions = Array.isArray(tableOptions.columns) && tableOptions.columns.length > 0
        ? tableOptions.columns
        : Array.from({ length: maxExplicitColumns }, () => ({ mode: 'flex', fr: 1 } as TableColumnSizing));
    const solved = solveTrackSizing({
    ...
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
        ...
        for (let cellIndex = 0; cellIndex < children.length; cellIndex += 1) {
            while (occupied.get(`${rowIndex}:${currentColumn}`)) {
                currentColumn += 1;
            }
    ...
    return deepSortObject({
        ...
        cells,
        blockStyle: Object.keys(blockStyle).length > 0 ? blockStyle : undefined,
        cellStyle: isObject(tableOptions.cellStyle)
            ? deepSortObject(toStyleRecord(tableOptions.cellStyle))
            : undefined,
        headerCellStyle: isObject(tableOptions.headerCellStyle)
            ? deepSortObject(toStyleRecord(tableOptions.headerCellStyle))
            : undefined,
...
export function readDocumentFixture(fixturePath: string): DocumentIR {
    const raw = JSON.parse(fs.readFileSync(fixturePath, 'utf8')) as DocumentInput;
    return resolveDocumentPaths(raw, fixturePath);
}

export function createSpatialFixture(document: DocumentIR, fixtureName: string): SpatialDocumentFixture {
    const pageGeometry = getPageGeometry(document);
...
function getFixtureNamesFromArgs(): string[] {
    ...
    if (args.length === 0) {
        return fs.readdirSync(REGRESSION_DIR)
            ...
            .filter((entry) => !entry.endsWith('.snapshot.layout.json'))
            .filter((entry) => !entry.endsWith('.spatial-ir.json'))
...
export function run(): void {
    ...
    for (const fixtureName of fixtureNames) {
        const fixturePath = path.join(REGRESSION_DIR, fixtureName);
        ...
        const spatialFixture = createSpatialFixture(document, fixtureName);
