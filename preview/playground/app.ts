...
> You are reading this document inside a live layout engine. Every word, every page break, every justified lineâ€”rendered in real-time by the same engine that produces your final PDF.
...
VMPrint was designed from the ground up to eliminate all of these trade-offs. It is a **self-contained layout engine**â€”no browser, no framework, no serverâ€”that produces publication-quality documents.
...
1. **Page navigation** â€” \`preview.getPageCount()\`, \`renderPageToCanvas(index, canvas)\`
2. **PDF export** â€” \`preview.exportPdf()\` returns a \`Uint8Array\`, fully in-browser, no upload required
3. **SVG export** â€” \`preview.exportSvgPage(index, { textMode: 'text' })\` for selectable text
4. **Live re-render** â€” \`preview.updateDocument(newAst)\` replaces the document with zero flicker

Try switching the **Style Preset** dropdown. The same markdown instantly reflows into a completely different documentâ€”different page size, font, column count, marginsâ€”without touching a single word.
...
The engine handles fonts asynchronouslyâ€”OpenType files are fetched on demand, cached, and re-settled into the layout automatically. The \`onFontProgress\` callback lets you show a progress indicator while it happens (you can see ours in the footer bar below).
...
This documentâ€”the one you are readingâ€”is not a static string embedded in the application. It is a Markdown file being processed live by the **@vmprint/mkd-mkd** transmuter. Every time you edit the source in the left panel, this happens:

\`\`\`
Your Markdown â†’ @vmprint/mkd-mkd â†’ DocumentInput AST â†’ Layout Engine â†’ Canvas
\`\`\`

Click the **{}** button to open the AST drawer. You will see the full DocumentInput JSON that the transmuter produced from this markdown. The engine has no knowledge that the source was Markdownâ€”it only sees a list of typed elements with style properties.
...
Pass this YAML directly into the transmuterâ€”no JavaScript object transformation, no build step:
...
Because the engine's input is a plain data structureâ€”just a JSON objectâ€”you can write a transmuter for absolutely any source format. The only requirement is that your function returns a valid **DocumentInput**.
...
No dependencies. No framework. Pass this to \`createVMPrintPreview\` and you have a live, paginated, exportable preview of your custom formatâ€”indistinguishable from anything built with the official transmuters.
...
- A **book publishing pipeline** where a single markdown manuscript is transmuted into a print-ready PDF, an e-reader EPUB, and a web previewâ€”all from the same source
- A **custom DSL** for your domainâ€”medical records, architectural specifications, academic transcriptsâ€”with layout rules baked into the transmuter, invisible to the author

react-pdf can produce beautiful PDFs. But it requires a React tree, JSX syntax, and a developer to maintain the layout code forever. A VMPrint transmuter is a pure function that any developer can understand in an afternoonâ€”and a YAML theme that a designer can own entirely.
...
A markdown document is being parsed, semantically analyzed, flowed through a full desktop-publishing layout engine with optical margin alignment, advanced justification, JIT font loading, and multi-pass paginationâ€”then rasterized to a retina-quality canvasâ€”in **under a second**, with **no server**, **no plugin**, and **~1.7MB** of JavaScript.

The same code that runs here runs on a Cloudflare Worker. The same code runs in a CI pipeline. The same code runs in a VS Code extension. The document you export is not a screenshot of a web pageâ€”it is a geometrically precise, font-embedded, fully archival PDF.
