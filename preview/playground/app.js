import { createVMPrintPreview } from "/playground/preview-local.mjs";
import { transmute } from "https://esm.sh/@vmprint/mkd-mkd@1.0.2?bundle";
const FONT_REPOSITORY_BASE_URL = "https://cdn.jsdelivr.net/npm/@vmprint/local-fonts@1.1.4/";
const DEFAULT_MARKDOWN = `# VMPrint Preview

> You are reading this document inside a live layout engine. Every word, every page break, every justified line\xE2\u20AC\u201Drendered in real-time by the same engine that produces your final PDF.
...
VMPrint was designed from the ground up to eliminate all of these trade-offs. It is a **self-contained layout engine**\xE2\u20AC\u201Dno browser, no framework, no server\xE2\u20AC\u201Dthat produces publication-quality documents.
...
1. **Page navigation** \xE2\u20AC\u201D \`preview.getPageCount()\`, \`renderPageToCanvas(index, canvas)\`
2. **PDF export** \xE2\u20AC\u201D \`preview.exportPdf()\` returns a \`Uint8Array\`, fully in-browser, no upload required
3. **SVG export** \xE2\u20AC\u201D \`preview.exportSvgPage(index, { textMode: 'text' })\` for selectable text
4. **Live re-render** \xE2\u20AC\u201D \`preview.updateDocument(newAst)\` replaces the document with zero flicker

Try switching the **Style Preset** dropdown. The same markdown instantly reflows into a completely different document\xE2\u20AC\u201Ddifferent page size, font, column count, margins\xE2\u20AC\u201Dwithout touching a single word.
...
The engine handles fonts asynchronously\xE2\u20AC\u201DOpenType files are fetched on demand, cached, and re-settled into the layout automatically. The \`onFontProgress\` callback lets you show a progress indicator while it happens (you can see ours in the footer bar below).
...
This document\xE2\u20AC\u201Dthe one you are reading\xE2\u20AC\u201Dis not a static string embedded in the application. It is a Markdown file being processed live by the **@vmprint/mkd-mkd** transmuter. Every time you edit the source in the left panel, this happens:

\`\`\`
Your Markdown \xE2\u2020\u2019 @vmprint/mkd-mkd \xE2\u2020\u2019 DocumentInput AST \xE2\u2020\u2019 Layout Engine \xE2\u2020\u2019 Canvas
\`\`\`

Click the **{}** button to open the AST drawer. You will see the full DocumentInput JSON that the transmuter produced from this markdown. The engine has no knowledge that the source was Markdown\xE2\u20AC\u201Dit only sees a list of typed elements with style properties.
...
Pass this YAML directly into the transmuter\xE2\u20AC\u201Dno JavaScript object transformation, no build step:
...
Because the engine's input is a plain data structure\xE2\u20AC\u201Djust a JSON object\xE2\u20AC\u201Dyou can write a transmuter for absolutely any source format. The only requirement is that your function returns a valid **DocumentInput**.
...
No dependencies. No framework. Pass this to \`createVMPrintPreview\` and you have a live, paginated, exportable preview of your custom format\xE2\u20AC\u201Dindistinguishable from anything built with the official transmuters.
...
- A **book publishing pipeline** where a single markdown manuscript is transmuted into a print-ready PDF, an e-reader EPUB, and a web preview\xE2\u20AC\u201Dall from the same source
- A **custom DSL** for your domain\xE2\u20AC\u201Dmedical records, architectural specifications, academic transcripts\xE2\u20AC\u201Dwith layout rules baked into the transmuter, invisible to the author

react-pdf can produce beautiful PDFs. But it requires a React tree, JSX syntax, and a developer to maintain the layout code forever. A VMPrint transmuter is a pure function that any developer can understand in an afternoon\xE2\u20AC\u201Dand a YAML theme that a designer can own entirely.
...
A markdown document is being parsed, semantically analyzed, flowed through a full desktop-publishing layout engine with optical margin alignment, advanced justification, JIT font loading, and multi-pass pagination\xE2\u20AC\u201Dthen rasterized to a retina-quality canvas\xE2\u20AC\u201Din **under a second**, with **no server**, **no plugin**, and **~1.7MB** of JavaScript.

The same code that runs here runs on a Cloudflare Worker. The same code runs in a CI pipeline. The same code runs in a VS Code extension. The document you export is not a screenshot of a web page\xE2\u20AC\u201Dit is a geometrically precise, font-embedded, fully archival PDF.
...
const markdownInput = byId("markdown-input");
const styleSelect = byId("style-preset");
const pdfButton = byId("pdf-button");
const svgButton = byId("svg-button");
const copyMarkdownButton = byId("copy-markdown-button");
const prevButton = byId("prev-button");
const nextButton = byId("next-button");
const pagerText = byId("pager-text");
const zoomInBtn = byId("zoom-in");
const zoomOutBtn = byId("zoom-out");
const zoomFitBtn = byId("zoom-fit");
const interactionModeBtn = byId("interaction-mode");
const zoomText = byId("zoom-text");
const statusNode = byId("status-bar");
const previewCanvas = byId("preview-canvas");
const renderViewport = byId("render-viewport");
const astOutput = byId("ast-output");
const fontProgressBar = byId("font-progress-bar");
const fontProgressContainer = byId("font-progress-container");
const toggleAstBtn = byId("toggle-ast-btn");
const closeAstBtn = byId("toggle-ast");
const astDrawer = byId("ast-drawer");
const saveStatus = byId("save-status");
const loader = byId("rendering-loader");
...
let currentDocumentAst = null;
let currentSnapshotPage = null;
...
const extractMarkdownImageSources = (markdown) => {
...
const fetchResolvedImage = async (src) => {
  try {
...
const primeImageCache = async (markdown) => {
...
const resolveImageFromCache = (src) => {
  const normalized = normalizeImageSrc(src);
  return imageCache.get(normalized) ?? null;
};
const onFontProgress = (event) => {
...
const normalizeSelectedSourceId = (value) => {
  const normalized = String(value || "").replace(/^gen:/, "").replace(/^author:/, "");
  return normalized || null;
};
const paintVisibleCanvas = () => {
...
const updateInteractionButton = () => {
  interactionModeBtn.textContent = interactionMode === "pan" ? "Pan" : "Select";
};
const copyCurrentSelectionAsMarkdown = async () => {
...
const processRender = async (resetZoom = false) => {
  ...
  try {
    ...
    if (!preview) {
      preview = await createVMPrintPreview(docAst, {
        fonts: { repositoryBaseUrl: FONT_REPOSITORY_BASE_URL },
        onFontProgress
      });
    } else {
...
previewCanvas.addEventListener("mousedown", (event) => {
  ...
  if (target) {
    ...
    activeTextSelection = dragAnchor ? preview.resolvePageSelection(currentPageIndex, dragAnchor, point, "continuous") : null;
